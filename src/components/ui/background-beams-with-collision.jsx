...
export const BackgroundBeamsWithCollision = ({ children, className }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        // Default dark gradient; override via className prop if needed
    ...
    >
      ...
      {/* “Ground” that beams collide with */}
      <div
        ref={containerRef}
        className="pointer-events-none absolute bottom-0 inset-x-0 w-full bg-neutral-900"
        style={{
          boxShadow:
...
const Explosion = (props) => {
  const spans = Array.from({ length: 20 }, (_, id) => ({
    id,
    initialX: 0,
    initialY: 0,
    ...
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      <motion.div
        ...
        className="absolute -inset-x-10 top-0 m-auto h-2 w-10 rounded-full
                   bg-gradient-to-r from-transparent via-indigo-500 to-transparent blur-sm"
      />
      {spans.map((s) => (
        <motion.span
          key={s.id}
